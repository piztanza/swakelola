...
// file_put_contents($filename, $image);
    ...
    $sql = "INSERT INTO users (created_at, updated_at, fullname, email, phone, role_id, instansi, id_kategori, status, status_id, vote_opportunity, kode_verifikasi, status_verifikasi) VALUES ('$dibuat', '$dibuat', '$fullname', '$email', '$phone', '3', '$instansi', '$kategori', '$status', '$status_id', 0, '$kode_verifikasi', 0)";
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        try {
            ...
            // handle verify email
            $emailSubject2 = 'Get Code Verification Email -' . $fullname;
            $emailBody2 = '
            <!DOCTYPE html>
            <html>
                <head>
            ...
            try {
                ...
                $mail->Host       = 'mail.pekanrisetsawit.id';                     // Set the SMTP server to send through
                $mail->SMTPAuth   = true;                                   // Enable SMTP authentication
                $mail->Username   = 'registrasi@pekanrisetsawit.id';               // SMTP username (your email)
                $mail->Password   = 'pekanrisetsawit2024';                  // SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption, `PHPMailer::ENCRYPTION_SMTPS` also accepted
                $mail->Port       = 587;                                    // TCP port to connect to

                //Recipients
                $mail->setFrom('registrasi@pekanrisetsawit.id', 'Pekan Riset Sawit 2024');      // Set the sender's email and name
                $mail->addAddress($emailRecipient);                         // Add a recipient (to)
                $mail->addBCC('registrasi@pekanrisetsawit.id');
                
                // Content
                $mail->isHTML(true);                                        // Set email format to HTML
                $mail->Subject = $emailSubject2;                            // Set the email subject
                $mail->Body    = $emailBody2;                               // Set the email body (HTML)
                $mail->AltBody = strip_tags($emailBody2);                   // Set the alternative plain text body (for non-HTML mail clients)
